...
AlgorithmResultTO *DFSBranchBound::process() {
    ...
    // Create the initial node and perform the first reduction
    auto first_node =  Matrix(0, matrix, vector<int>(), 0);
...
pair<int, std::vector<int>> DFSBranchBound::nearest_neighbor_cost(const vector<vector<int>> &graph, vector<bool> &visited, int current) {
